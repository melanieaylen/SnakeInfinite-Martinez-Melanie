...
public class PantallaJuegoMultijugador implements Screen {
    ...
    private int[] puntuaciones = {0, 0, 0, 0};
    private int[] vidas = {3, 3, 3, 3};
    ...
    private void procesarEntradas() {
        if (juegoTerminado || alguienSeDesconecto) return;
    ...
    private void renderizarUI() {
        ...
        if (alguienSeDesconecto) {
            textoDesconectado.dibujarTexto("Un jugador se desconectó", 
                                          Config.ANCHO / 2 - 280, Config.ALTO / 2 + 50);
            textoDesconectado.dibujarTexto("Volviendo al menú...", 
                                          Config.ANCHO / 2 - 200, Config.ALTO / 2 - 20);
        } else if (juegoTerminado) {
            textoGanador.dibujarTexto("¡Jugador " + obtenerGanador() + " GANÓ!", 
                                     Config.ANCHO / 2 - 250, Config.ALTO / 2);
        }
    ...
    public void cuandoActualizanSerpiente(int numeroJugador, String datosSegmentos) {
        ...
        // ✅ DEBUGGING cada 100 actualizaciones
        if (actualizacionesRecibidas % 100 == 0) {
            System.out.println("📥 [Cliente J" + miNumeroJugador + "] Actualización #" + actualizacionesRecibidas);
    ...
    public void cuandoActualizanFrutas(String datosFrutas) {
        ...
        if (datosFrutas == null || datosFrutas.isEmpty()) {
            System.out.println("⚠️ Sin datos de frutas");
            return;
        }
    ...
    public void cuandoJugadorMuere(int numeroJugador, int vidasRestantes) {
        ...
        if (indice >= 0 && indice < MAX_JUGADORES) {
            ...
            System.out.println("💀 [Cliente J" + miNumeroJugador + "] J" + numeroJugador + 
                             " murió (quedan " + vidasRestantes + " vidas)");
        }
    }
    ...
    public void cuandoTerminaJuego(int ganador) {
        juegoTerminado = true;
        System.out.println("🏆 [Cliente J" + miNumeroJugador + "] Juego terminado, ganó J" + ganador);
    ...
    public void cuandoRivalSeDesconecta(int numeroJugador) {
        ...
        System.out.println("⚠️ [Cliente J" + miNumeroJugador + "] J" + numeroJugador + " se desconectó");
    ...
    public void dispose() {
        ...
        if (textoGanador != null) textoGanador.dispose();
        if (textoDesconectado != null) textoDesconectado.dispose();
    }
}